...
public class UserServiceWithGroupsImpl implements UserServiceWithGroups {
  ...
  public void addGroup(BasicGroup group) {
    ...
    if (findGroupByName(group.getMembers().iterator().next().getName(), group.getName())
        .isPresent()) {
  ...
  public synchronized void deleteGroup(User sender, BasicGroup group)
      throws SenderNotAuthorizedException {
    Session session = sessionFactory.openSession();
    session.beginTransaction();
  ...
  /**
   * Adds a user. Checks for existence have been performed by the controller. Checking if allowed,
   * and persisting change. Any member can add.
   *
  ...
  public synchronized void extendUsers(User sender, User user, BasicGroup group)
      throws SenderNotAuthorizedException, UserAlreadyPresentException {
  ...
  /**
   * Adds a user. Checks for existence have been performed by the controller. Checking if allowed,
   * and persisting change. Any Moderator can add. Adds user as member if not already.
   *
   * @param sender - User sender of message, stored at user login.
   * @param user   - User to be added.
   * @param group  - Basic group to be changed.
  ...
  /**
   * Removes a user. Checks for existence have been performed by the controller. Checking if
   * allowed, and persisting change. Any Moderator can remove. Users can remove themselves.
   *
   * @param sender - User sender of message, stored at user login.
   * @param user   - User to be added.
   * @param group  - Basic group to be changed.
  ...
  public synchronized void removeUser(User sender, User user, BasicGroup group)
    ...
    if (!members.contains(user)) {
      throw new SenderNotAuthorizedException("No such user in users");
    }
  ...
  /**
   * Removes a moderator. Checks for existence have been performed by the controller. Checking if
   * allowed, and persisting change. Can only remove if sender and moderator. If last moderator
   * removed, group it deleted even if users are present.
   *
   * @param sender - User sender of message, stored at user login.
   * @param user   - User to be added.
   * @param group  - Basic group to be changed.
  ...
  public synchronized void removeModerator(User sender, User user, BasicGroup group)
    ...
    if (!moderators.contains(user)) {
      throw new SenderNotAuthorizedException("user not a moderator");
    }
